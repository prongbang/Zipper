...
public typealias ZipperCRC32 = UInt32
...
public typealias ZipperConsumerClosure = (_ data: Data) throws -> Void
...
public typealias ZipperProviderClosure = (_ position: Int, _ size: Int) throws -> Data

/// The lookup table used to calculate `ZipperCRC32` checksums.
public let ZipperCRCTable: [UInt32] = [
...
extension Data {
    ...
    /// Calculates the `ZipperCRC32` checksum of the receiver.
    ///
    /// - Parameter checksum: The starting seed.
    /// - Returns: The checksum calcualted from the bytes of the receiver and the starting seed.
    @inline(__always)
    public func crc32(checksum: ZipperCRC32) -> ZipperCRC32 {
        ...
        self.withUnsafeBytes { (bytes: UnsafePointer<UInt8>) in
            ...
            for chunk in chunks {
                for byte in chunk {
                    ...
                    result = (result >> 8) ^ ZipperCRCTable[index]
                }
            }
        }
        return result ^ mask
    ...
    static func compress(size: Int, bufferSize: Int, provider: ZipperProviderClosure, consumer: ZipperConsumerClosure) throws -> ZipperCRC32 {
    ...
    static func decompress(size: Int, bufferSize: Int, provider: ZipperProviderClosure, consumer: ZipperConsumerClosure) throws -> ZipperCRC32 {
#if os(macOS) || os(iOS) || os(watchOS) || os(tvOS)
...
extension Data {
    ...
    static func process(operation: compression_stream_operation, size: Int, bufferSize: Int,
                        provider: ZipperProviderClosure, consumer: ZipperConsumerClosure) throws -> ZipperCRC32 {
        var position = 0
    ...
    @inline(__always)
    static func fill(stream: inout compression_stream, checksum: inout ZipperCRC32, chunk: inout Data,
                     progress: (advance: (position: Int, readSize: Int),
                     configuration: (operation: compression_stream_operation, flags: Int32)),
                     provider: ZipperProviderClosure) throws -> Int {
    ...
    @inline(__always)
    static func flush(stream: inout compression_stream, checksum: inout ZipperCRC32,
                      buffer: (size: Int, pointer: UnsafeMutablePointer<UInt8>),
                      operation: compression_stream_operation, consumer: ZipperConsumerClosure) throws {
...
extension Data {
    @inline(__always)
    static func encode(size: Int, bufferSize: Int, provider: ZipperProviderClosure, consumer: ZipperConsumerClosure) throws -> ZipperCRC32 {
        var stream = z_stream()
    ...
    @inline(__always)
    static func decode(bufferSize: Int, provider: ZipperProviderClosure, consumer: ZipperConsumerClosure) throws -> ZipperCRC32 {
        var stream = z_stream()
        let streamSize = Int32(MemoryLayout<z_stream>.size)
        var result = inflateInit2_(&stream, -MAX_WBITS, ZLIB_VERSION, streamSize)
        defer { inflateEnd(&stream) }
        guard result == Z_OK else { throw CompressionError.invalidStream }
        var unzipCRC32 = ZipperCRC32(0)
        var position = 0
        repeat {
